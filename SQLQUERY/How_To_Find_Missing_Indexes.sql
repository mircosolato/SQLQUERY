...
SELECT gs.[avg_user_impact] AS [AvgUserImpact]  -- The value means that the query cost would on average drop by this percentage if this missing index group was implemented.
...
--AND id.[inequality_columns] IS NULL
AND id.[included_columns] IS NULL
--ORDER BY gs.[user_seeks] DESC
ORDER BY gs.[avg_user_impact] DESC, gs.[avg_total_user_cost] DESC
OPTION (RECOMPILE);
